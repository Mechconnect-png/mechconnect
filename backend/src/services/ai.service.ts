...
export interface AIQuestionnaireInput {
  vehicleType?: string;
  symptomsText?: string;
...
export class AIService {
  public static diagnose(input: AIQuestionnaireInput): AIDiagnosisResult {
    ...
    if (
      (startsEngine === false || stoppedSuddenly) &&
    ...
    if (
      stoppedSuddenly && (text.includes("accident") || text.includes("hit") || text.includes("towing") || text.includes("breakdown") || text.includes("stuck"))
    ) {
      return {
